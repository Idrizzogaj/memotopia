...
export function flatten(validationErrors: ValidationError[], prefix: string = ''): FieldErrors {
    ...
    for (const { property, children, constraints, value } of validationErrors) {
        ...
        if (children) {
            fieldErrors = { ...fieldErrors, ...flatten(children, `${prefix}${name}.`) };
        }
    }
    return fieldErrors;
}
...
export class IsAfterConstraint implements ValidatorConstraintInterface {
    ...
    defaultMessage(args: ValidationArguments) {
        return `Must be after ${args.constraints[0]}`;
    }
}
