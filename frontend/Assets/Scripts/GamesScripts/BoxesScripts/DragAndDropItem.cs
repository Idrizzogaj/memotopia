...
public class DragAndDropItem : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    private static string canvasName = "DragAndDropCanvas";                         // Sort order for canvas
    ...
    void Awake()
    {
        ...
        //Canvas defaultCanvas = GameObject.FindGameObjectWithTag("Canvas").GetComponent<Canvas>();
        //Camera cam = GameObject.FindGameObjectWithTag("MainCamera").GetComponent<Camera>();
        //if (defaultCanvas != null)
        //{
        //    canvas = defaultCanvas;
        //    //canvas.sortingOrder = canvasSortOrder;
        //    //canvas.transform.position = new Vector3(0, 0, -12000);
        //}
    ...
    public void OnBeginDrag(PointerEventData eventData)
    {
        if (dragDisabled == false)
        {
            ...
            draggedItem.GetComponent<Image>().type = Image.Type.Filled;
            draggedItem.GetComponent<Image>().fillAmount = 0;
            // Create item's icon
            icon = new GameObject();
            icon.transform.SetParent(canvas.transform);
            icon.name = draggedItem.name;
    ...
    public void OnDrag(PointerEventData data)
    {
        if (icon != null)
        {
            ...
            icon.transform.position = new Vector3(Input.mousePosition.x, Input.mousePosition.y, canvas.transform.position.z);
            icon.transform.SetParent(canvas.transform);
        }
    }
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        try
        {
            draggedItem.GetComponent<Image>().fillAmount = 1;
        }
    ...
    public void ChangeImageTypeToSimple() {
        this.GetComponent<Image>().type = Image.Type.Simple;
    }
